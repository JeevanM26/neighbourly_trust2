...
/**
 * Register background Service Worker and obtain FCM device push token for worker
 */
export async function registerFcmToken(workerId: string, vapidKey?: string): Promise<string | null> {
  ...
  try {
    ...
    console.warn('[FCM Worker] Token registration notice:', err);
    return null;
  }
}
